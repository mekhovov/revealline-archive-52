"""Read-only, fail-closed intake of the actual Archive52 initial-deployment receipts."""
import hashlib
import io
import json
import os
...
BASE = 'https://mekhovov.github.io/revealline-archive-52/'
REPO = 'mekhovov/revealline-archive-52'
TREE = 'b21a8bdf97a7b4af190f065286d470e2c73e1f4f'
INVENTORY_SHA = 'b3d2d33d0428cf2668b1c5a46978a12d9122ed30a88dec0e6e4c51ee7f48e4d0'
LOCK_SHA = 'ba2ff6192a10d2c06631f43317d1c11d3dc104f5447cce130e6e3e72011d557b'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1063 and sum(r['bytes'] for r in rows) == 590419655, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive52-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive52-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.83.0.json']
